...
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT * from user where accountno=$from";
    $query = mysqli_query($conn,$sql);
    $sql1 = mysqli_fetch_array($query); // returns array or output of user from which the amount is to be transferred.

    $sql = "SELECT * from user where accountno=$to";
    $query = mysqli_query($conn,$sql);
    ...
    if (($amount)<0)
   {
    ...
    // constraint to check zero values
         ...
         echo "</script>";
                ...
                $sql = "UPDATE user set balance=$newbalance where accountno=$from";
                mysqli_query($conn,$sql);
             

                // adding amount to reciever's account
                $newbalance = $sql2['balance'] + $amount;
                $sql = "UPDATE user set balance=$newbalance where accountno=$to";
                mysqli_query($conn,$sql);
                ...
                $receiver = $sql2['name'];
                $r1 = $sql2['accountno'];
                $sql = "INSERT INTO trans(`sender`, `receiver`,`saccount`,`raccount`, `balance`,`date`) VALUES ('$sender','$receiver','$s1','$r1','$amount','$d1')";
                $query=mysqli_query($conn,$sql);
	...
	<div class="container">
        <h2 class="text-center pt-4" style="color : black;">Transaction</h2>
            <?php
                include 'config.php';
                $sid=$_GET['id'];
                $sql = "SELECT * FROM  user where accountno=$sid";
        ...
        <select name="to" class="form-control" required>
            ...
            ?>
                <option class="table" value="<?php echo $rows['accountno'];?>" >
                    ...
                    <?php echo $rows['accountno'] ;?> &nbsp Mobile:<?php echo $rows['mobile'];?> ) 
